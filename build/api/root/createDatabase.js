...
import { createUpstashRedisDatabase, getUpstashRedisDatabase, } from "../../src/upstashRedis";
import { generateId, notEmpty, tryParseJson, } from "from-anywhere";
import { rootDatabaseName } from "../../src/state";
import { embeddingsClient } from "../../src/embeddings";
export const createDatabase = async (context) => {
    const { databaseSlug, schemaString, authToken, region, X_UPSTASH_API_KEY, X_UPSTASH_EMAIL, X_ADMIN_AUTH_TOKEN, X_OPENAPI_API_KEY, vectorIndexColumns, } = context;
    ...
    let previousDatabaseDetails = await root.get(realDatabaseSlug);
    const schema = tryParseJson(schemaString);
    if (!schema) {
        return { isSuccessful: false, message: "Invalid Schema" };
    }
    if (["root", rootDatabaseName].includes(realDatabaseSlug) ||
    ...
    if (!previousDatabaseDetails) {
        // creates indexes
        const vectorIndexColumnDetails = vectorIndexColumns && upstashApiKey && upstashEmail
    ...
    return {
        ...
        openapiUrl: "https://data.actionschema.com/" + realDatabaseSlug + "/openapi.json",
    };
};
